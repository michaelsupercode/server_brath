...
} = require("./use-cases/add-product-to-user-wishlist");
...
app.get("/api/products/all", async function getAllProductsController(_, res) {
  try {
    ...
    res
      .status(500)
      .json({ err: error.message || "Unknown error while reading products." });
  }
});
...
app.get("/api/products/single/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ err: error.message || "Unknown error while reading product." });
  }
});
...
app.get("/api/users/userInfo", doAuthMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        err: error.message || "Unknown error while getting your user info.",
      });
  }
});
  ...
  try {
    ...
    res
      ...
      .json({
        err:
          error.message ||
          "Unknown error while adding product to user whishlist.",
      });
  }
});
